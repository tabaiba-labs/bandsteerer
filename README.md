...
Download the latest `BandSteerer-<version>.dmg` from [GitHub Releases](https://github.com/tabaiba-labs/bandsteerer/releases/latest), open it, and drag **BandSteerer** to the **Applications** shortcut. Published DMGs are Developer ID signed, notarized by Apple, and stapled for Gatekeeper.
...
Public releases are deliberately produced on a maintainer's Mac instead of giving GitHub Actions access to Apple signing or notarization credentials. The maintainer archives the exact release commit with Xcode, exports a Developer ID build from Organizer, and uses the Apple command-line tools included with Xcode to create, sign, notarize, staple, and verify a universal drag-and-drop DMG. The finished DMG and its SHA-256 checksum are then attached to an annotated `vMAJOR.MINOR.PATCH` GitHub Release.

The repository contains no release script, alternate build definition, account identifier, certificate, or private key. GitHub Actions is limited to unprivileged CI. The complete, copy-and-paste release procedure is in [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md).
